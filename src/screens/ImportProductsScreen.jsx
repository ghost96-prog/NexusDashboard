...
const ImportProductsScreen = () => {
  ...
  const [existingProducts, setExistingProducts] = useState([]); // ADD THIS LINE
useEffect(() => {
...
const fetchExistingProducts = async () => {
  ...
  const validateCSV = (data) => {
    ...
    //   }
      ...
      if (row["Product SKU"] && isNaN(parseInt(row["Product SKU"]))) {
        errors.push(`Row ${rowNum}: Product SKU must be a number`);
      }
    });
  ...
  const getNextSKU = async (userEmail, userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://nexuspos.onrender.com/api/productRouter/products?email=${encodeURIComponent(userEmail)}`,
      ...
      if (response.ok) {
        ...
        userProducts.forEach(product => {
          const sku = parseInt(product.sku);
          if (!isNaN(sku) && sku > highestSKU) {
            highestSKU = sku;
          }
        });
...
const createProductsFromCSV = async () => {
  ...
  try {
    ...
    // Check for empty SKUs and duplicates like Android
    ...
    for (const item of csvData) {
      ...
      if (skuSet.has(trimmedSKU)) {
        toast.error("Duplicate SKUs found. Please ensure all SKUs are unique.");
        setIsUploading(false);
        return;
      }
    ...
    if (hasEmptySKU) {
      toast.error("Some products have empty SKUs. Please check your data.");
      setIsUploading(false);
      return;
    }
    
    // Process in chunks like Android (100 items per chunk)
    const chunkSize = 100;
    const chunks = [];
    ...
    let nextSKU = await getNextSKU(userEmail, userId);
    ...
    for (const chunk of chunks) {
      // Handle categories first - EXACTLY like Android
      for (const item of chunk) {
        ...
        if (!categoryMap.has(categoryName)) {
          ...
          // Save category to server - SAME ENDPOINT as Android
          try {
            const response = await fetch(
      ...
      // Create products - EXACTLY like Android logic
      for (const item of chunk) {
        const categoryName = item["Category"].toUpperCase();
        ...
        if (product.sku.trim() === "") {
          product.sku = nextSKU.toString();
          nextSKU++;
        }
        
        // Check if product exists - ONLY by productId
        let stockBefore = 0;
        ...
        try {
          ...
          // Create inventory update - EXACTLY like Android
          const inventoryUpdateData = {
            ...
            EditorId: userId,
            userId: userId,
  editedBy: isUpdate ? "adminApp" : "", // FIXED: Empty string for create, "adminApp" for update
          ...
          // Save inventory update - SAME ENDPOINT as Android
          await fetch(
      ...
      <div className="import-notes">
        <div className="note-card">
          ...
          <ul>
            ...
            <li><strong>Weight Products / Measured items (e.g items sold per meter or per kg):</strong> Use decimals for stock (e.g., 50.50)</li>
            <li><strong>Each Products:</strong> Use whole numbers for stock</li>
            <li><strong>Track Stock:</strong> Use "TRUE" or "FALSE"</li>
            <li><strong>SKU:</strong> Must be unique numbers</li>
